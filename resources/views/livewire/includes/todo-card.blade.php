<div wire:key='{{ $todo->id }}'
    class="todo mb-5 card px-5 py-6 {{ $todo->completed == 1 ? 'bg-green-200' : 'bg-white' }} col-span-1 border-t-2 border-blue-500 hover:shadow">
    <div class="flex justify-between space-x-2">

        <div class="flex items-center">
            @if ($todo->completed)
                <input wire:click='toggleCompleted({{ $todo->id }})' class="mr-2" type="checkbox" checked>
            @else
                <input wire:click='toggleCompleted({{ $todo->id }})' class="mr-2" type="checkbox">
            @endif
